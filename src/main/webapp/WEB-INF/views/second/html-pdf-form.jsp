...
<body>
    ...
    <script>
        // 샘플 html 코드 자동완성 (4가지 제공중 basic, table, form, korean-test)
        //	basic -> 기본 p태그와 리스트 요소
        //	table -> table 태그 요소
        //	form -> form태그 입력 양식예제를 통해 데이터를 구성했다고 가정 (div 와 h2, p 태그로 변환)
        //	korean-test -> 한글 입력 테스트 (폰트적용해야함)
...
<div class="korean-test">
    ...
    <p>주소: 서울특별시 강남구 테헤란로 123</p>
        ...
        // 샘플 CSS 로드 (4가지 제공중 basic, modern, print, korean)
        // basic : 베이직함
        // modern : 테이블 헤드th 태그에 색상 추가 적용
        // print : 프린트에 적합한 색상
        // korean : html예시 korean-test 에 맞게 구성됨
...
em {
    ...
    color: #e74c3c;
        ...
        // 로딩 상태 표시 -> 요청 보내고 해당 요청에 대한 응답이 마무리 되기 전까지 표시할 UI
        ...
        // PDF 미리보기 -> 새 윈도우 창으로 오픈
        function previewPdf() {
            showLoading();
            
            const formData = new FormData(document.getElementById('pdfForm'));	// html요소 문자열, css요소 문자열로 FormData 객체 생성
...
//             console.log("css : ", cs);
            ...
            fetch('/pdf/preview', {
                method: 'POST',
                body: formData													// content-type 디폴트로 application/x-www-form-urlencoded
            })
            .then(response => {													// 서버에서 pdf파일 동적으로 만들고 byte로 변환 후 resource로 보내줌
                if (!response.ok) {
                    throw new Error('미리보기 요청 실패');
                }
                return response.blob();											// 서버에서 받은 resource는 binary데이터. 해당 데이터를 꺼냄
            })
            .then(blob => {
                const url = window.URL.createObjectURL(blob);					// binary데이터를 URL 객체로 변환 -> href처럼 사용됨.
                ...
                // 새 창에서 PDF 열기
                const previewWindow = window.open(pdfUrlWithZoom, 'pdfPreview', windowFeatures);	// 팝업으로(새 윈도우) 열기
                
                if (!previewWindow) {
                    window.open(pdfUrlWithZoom, '_blank');											// 팝업 차단 되었으면 새로운 탭에서 열기
                }
        ...
        function downloadPdf() {
            ...
            const form = document.getElementById('pdfForm');								// html소스와 css소스가 있는 form요소
            form.action = '/pdf/generate';													// action url값 변경
            form.submit();																	// 전송 (post임)
        ...
        function testKoreanPdf() {
            ...
            const form = document.getElementById('pdfForm');								// html소스와 css소스가 있는 form요소
            form.action = '/pdf/generate-korean';													// action url값 변경
            form.submit();																	// 전송 (post임)
